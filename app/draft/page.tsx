...
const mockDraftedPlayers: any[] = []
const mockQueuePlayers: any[] = []
...
export default function DraftCommandCenter() {
  ...
  const [queuedPlayers, setQueuedPlayers] = useState<Set<number>>(new Set())
  const [messages, setMessages] = useState<any[]>([])
  const messagesEndRef = useRef<HTMLDivElement>(null)

  const currentSidekick = selectedSidekick ? sidekickData[selectedSidekick as keyof typeof sidekickData] : null
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.error("Failed to get SideKick response:", error)
    }
  }
                    ...
                    {currentSidekick ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center text-muted-foreground">
                          <p className="font-body text-sm">No SideKick selected</p>
                          <p className="font-body text-xs mt-2">Please select a SideKick to continue</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <CardHeader className="pb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={handleEndDraft} className="text-xs bg-transparent">
                        End Draft
                      </Button>
                      <Button variant="outline" size="sm" onClick={handleClearDraft} className="text-xs bg-transparent">
                        Clear Draft
                      </Button>
                  ...
                  <div className="max-h-[400px] overflow-y-auto">
                    {filteredPlayers.length === 0 ? (
                      <div className="text-center py-8 text-muted-foreground">
                        <p className="font-body text-sm">No players available</p>
                        <p className="font-body text-xs mt-2">Upload draft data to see player rankings</p>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        <div className="grid grid-cols-12 gap-2 text-xs font-subheading text-muted-foreground border-b border-border pb-2 sticky top-0 bg-card z-10">
            ...
            <div className="lg:col-span-3">
              <div className="space-y-6">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-center py-4 text-muted-foreground">
                      <p className="font-body text-sm">No alerts available</p>
                      <p className="font-body text-xs mt-2">Draft alerts will appear here during active drafts</p>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    {mockDraftedPlayers.length === 0 ? (
                      <div className="text-center py-4 text-muted-foreground">
                        <p className="font-body text-sm">No picks yet</p>
                        <p className="font-body text-xs mt-2">Your drafted players will appear here</p>
                      </div>
                    ) : (
                      <div className="space-y-3">
