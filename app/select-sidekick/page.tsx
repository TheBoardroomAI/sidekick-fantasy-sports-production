...
// Mock user tier - would come from backend
const userTier = "" // Empty string means no plan selected
...
export default function SelectSidekickPage() {
  ...
  const [fetchedUserTier, setFetchedUserTier] = useState("")
  ...
  useEffect(() => {
    ...
    axios
      .get("/api/user-tier")
      .then((response) => {
        setFetchedUserTier(response.data.userTier)
      })
      .catch((error) => {
  ...
  const availableSidekicks = fetchedUserTier
    ? sidekicks.filter((sidekick) => sidekick.availableFor.includes(fetchedUserTier))
    : []
  const lockedSidekicks = fetchedUserTier
    ? sidekicks.filter((sidekick) => !sidekick.availableFor.includes(fetchedUserTier))
    : sidekicks

  const handleSelectSidekick = (sidekickId: string) => {
    const sidekick = sidekicks.find((s) => s.id === sidekickId)
    if (sidekick && !sidekick.availableFor.includes(fetchedUserTier)) {
  ...
  const handleContinueWithSidekick = () => {
    if (selectedSidekick) {
  ...
  const handleNameSubmit = () => {
    ...
    // Store the selected sidekick and user name in localStorage for persistence
    localStorage.setItem("selectedSidekick", selectedSidekick!)
    localStorage.setItem("userTier", fetchedUserTier)
    localStorage.setItem("userName", userName.trim())

    // Navigate to SideKick Central
    window.location.href = `/dashboard?sidekick=${selectedSidekick}`
  }
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          ...
          {fetchedUserTier ? (
            <Badge variant="secondary" className="font-subheading">
              {fetchedUserTier} Plan - {availableSidekicks.length} SideKicks Available (No Voice Features)
            </Badge>
          ) : (
            <Badge variant="secondary" className="font-subheading">
              Please select a plan to access SideKicks
            </Badge>
          )}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto mb-12">
          {sidekicks.map((sidekick) => {
            const isLocked = !fetchedUserTier || !sidekick.availableFor.includes(fetchedUserTier)
            return (
              <Card
                key={sidekick.id}
              ...
              >
                ...
                <CardContent className="space-y-4">
                  {/* Philosophy */}
                  <div className="p-3 rounded-lg bg-sidekick-success-green/5">
        ...
        {selectedSidekick && (
          <div className="text-center">
            <Button
              onClick={handleContinueWithSidekick}
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-lg p-6 max-w-md w-full mx-4 relative">
              ...
              <div className="text-center mb-6">
                <h3 className="font-headline text-xl mb-2">What should I call you?</h3>
                <p className="font-body text-muted-foreground">
                  Your SideKick wants to know how to address you during your fantasy journey.
                </p>
              </div>

              <div className="space-y-4">
                <div>
                  <Label htmlFor="userName" className="font-subheading">
                    Your Name *
                  </Label>
                  <Input
                    id="userName"
                    ...
                    autoFocus
                  />
                  {nameError && <p className="text-red-500 text-sm mt-1 font-body">{nameError}</p>}
                </div>

                <Button
                  onClick={handleNameSubmit}
                  className="w-full gradient-green-blue text-white font-subheading"
                  disabled={!userName.trim()}
                >
                  Continue to SideKick Central
                </Button>
              </div>
            </div>
          </div>
